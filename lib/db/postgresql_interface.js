...
PostgresqlInterface.factory = function factory() { return new PostgresqlInterface(); };

PostgresqlInterface.prototype.summonTransaction = 
	function summonTransaction(deviceId, tx, cb) {
	// First do an INSERT
	// If it worked, go ahead with transaction
	// If duplicate, fetch status and return
	var self = this;
	this.client.query('INSERT INTO transactions (id, status, deviceId, ' +
			'toAddress, satoshis, currencyCode, fiat, rate) ' + 
			'VALUES ($1, $2, $3, $4, $5, $6, $7, $8)', [tx.txId, 'pending', deviceId,
			tx.toAddress, tx.satoshis, tx.currencyCode, tx.fiat, tx.rate], 
			function (err) {
